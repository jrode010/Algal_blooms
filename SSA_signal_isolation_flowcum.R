...
# Load data
  dat <- read.csv(file = 'Data/cumflow_am.csv')

  str(dat)
  
  dat <- dat %>% dplyr::filter(date > ymd('2016-02-01'))
  dat <- dat %>% dplyr::filter(date < ymd('2024-03-01'))
...
##
  # Specify signal component groups
    ...
    #              , c(10,11)
    #                 )
    grp <- list( c(1,2), c(3,6), c(4,5,7,12), c(8,11), c(9,10)) #aflow
    grp <- list( c(1,2), c(3,4,5,6), c(7,8,9,11, 13, 14), c(10,12)) #mflow
...
##
 ...
 ####   
    signal
mflow_cum<- data.frame(signal) %>% setNames('mflow_cum')

allssa <- cbind(aflow_cum, mflow_cum)

write.csv(allssa, file = 'SSA_flowcum.csv')
