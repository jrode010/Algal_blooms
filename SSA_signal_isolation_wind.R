...
#### Load data#####
  dat <- read.csv(file = 'Data/Clean/monthly_wind_kw.csv')
  str(dat)
  dat <- dat %>% mutate(date = ymd(month))
  #Change character column to date
  dat <- dat %>%
    filter(date <= ymd('2024-02-01'))
  dat1 <- dat %>%
    filter(date <= ymd('2024-02-01')) %>% 
    select(c(mean_WSPD, mean_WDIR))
...
##
  # Specify signal component groups
    ...
    #              , c(10,11)
    #                 )
    grp <- list(c(1,2), c(3,4,5,6)) #speed
    grp <- list( c(1,2), c(3,4), c(5,6,7,8)) #direction
...
##
 ...
 ####   
    signal
dir <- data.frame(signal) %>% setNames('dir')

allssa <- cbind(speed, dir)

write.csv(allssa, file = 'SSA_wind.csv')

ssadates <- cbind(allssa, dat$date) %>% rename(date = `dat$date`)
write.csv(ssadates, file = 'SSA_wind_dates.csv')
